...
hidenum=41;%Number of neurons in the hidden layer
...
validdata=fold1;
traindata=[fold2;fold3;fold4;fold5;fold6;fold7;fold8;fold9;fold10];
Yvalid=[Ytrain(1:74,:);Ytrain(737:836,:)];
Ytrain=[Ytrain(75:148,:);Ytrain(837:936,:);
    Ytrain(149:222,:);Ytrain(937:1036,:);
...
Lvalid=[Ltrain(1:74,:);Ltrain(737:836,:)];
Ltrain=[Ltrain(75:148,:);Ltrain(837:936,:);
    Ltrain(149:222,:);Ltrain(937:1036,:);
...
for ii=1:max_epoch
    ...
    %Iteration of w2
    w2=pinv(Qtrain)*softinv(ytrain-alpha.*sinh(errortrain));
    
    %Validation
    hi_valid=soft(validdata(:,2:513)*w1);
    for i=1:hidenum
        Qvalid(:,i)=hi_valid(:,i).^(i-1);
    end
    yvalid=soft(Qvalid*w2);
